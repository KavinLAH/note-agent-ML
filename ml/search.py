...
class HybridSearchEngine:
    ...
    def index_chunk(self, chunk_id: str, text: str, vector: List[float]):
        ...
        Example distances in embedding space:
        - "Hire engineers" ↔ "Recruit developers" → Distance: 0.15 (very similar)
        - "Hire engineers" ↔ "Budget approval"     → Distance: 0.65 (somewhat related)
        - "Hire engineers" ↔ "Quantum physics"     → Distance: 0.92 (unrelated)
        
        When user searches "recruiting talent", the system finds spans with vectors
        close to the query's vector, even if exact words don't match.
        """
        self.chunks.append({
            "id": chunk_id,
            "text": text,
